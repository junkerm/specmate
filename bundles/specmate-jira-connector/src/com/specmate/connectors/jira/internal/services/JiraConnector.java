...
public class JiraConnector implements IRequirementsSource {
	...
	private List<Issue> getStoriesForEpic(Issue epic) {
		return this.getIssues("project=" + projectName + " AND issueType=story AND \"Epic Link\"=\"" + epic.getKey() + "\" ORDER BY assignee, resolutiondate");
	}
	
	private List<Issue> getStoriesWithoutEpic() {
		return this.getIssues("project=" + projectName + " AND issueType=story AND \"Epic Link\" IS EMPTY ORDER BY assignee, resolutiondate");
	}
	...
	private List<Issue> getIssues(String jql) {
		...
		while(issues.size() < maxResults) {
			try{
				SearchResult searchResult = jiraClient.getSearchClient().searchJql(jql, -1, issues.size(), null).claim();
		...
		logService.log(LogService.LOG_INFO, "Finished loading of " + issues.size() + " issues from Jira " + url + " project: " + projectName);
	...
	public boolean authenticate(String username, String password) throws SpecmateException {
		try {
			JiraRestClient client = new AsynchronousJiraRestClientFactory().createWithBasicHttpAuthentication(new URI(url), username, password);
			...
			if(status == 401) {
				logService.log(LogService.LOG_INFO, "Invalid credentials provided for jira project " + this.projectName);
				return false;
			}
			e.printStackTrace();
			return false;
